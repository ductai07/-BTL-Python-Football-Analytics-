...
def upload_video():
    # Xóa tất cả các file video upload cũ
...
def video_list():
    # Lấy tất cả các folder trong thư mục results
    ...
    if request.method == 'POST':
        # Lấy folder được chọn bởi người dùng
        ...
        selected_folder = folders[0]  # Mặc định chọn folder đầu tiên nếu chưa có lựa chọn

    # Đường dẫn tới folder được chọn
    folder_path = os.path.join(results_path, selected_folder)
    videos = [f for f in os.listdir(folder_path) if f.endswith(('.mp4', '.avi'))]

    # Chuyển đổi các video AVI sang MP4
    for video in videos:
        if video.endswith('.avi'):
            ...
            os.remove(avi_path)  # Xóa file AVI sau khi chuyển đổi

    # Cập nhật danh sách video sau khi chuyển đổi
